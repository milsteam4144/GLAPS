...
class allAttributes_T(Base):
    __tablename__ = "all_Attributes"
    year = Column('year', Integer, primary_key = True, nullable=False)
    stadiumExists = Column('StadiumExists', Integer)
    stateCode = Column('StateCode', String, primary_key=True)
    countyCode = Column('CountyCode', String, primary_key = True)
    ...
    def __init__(self, year, stateCode, countyCode, meanIncome, medianIncome, povertyPop, medianAge, workers, medianHouseIncome, medianFamilyIncome, medianNonFamIncome, medianWorkerIncome, \
                population, medianRealEstateTax, medianHouseholdCosts, totalHouses, medianHomeVal, stadiumExists):
        self.year = year
        self.stadiumExists = stadiumExists
        self.stateCode = stateCode
        self.countyCode = countyCode
...
listLocations = list(session.query(StatesAndCounties_T.stateAndCounty).all())
...
for location in listLocations:
    new = []
    for years in AllDetailedCounties:
        for each in years:
            #each[6] = "('" + each[6]+ "',)"
            #print (str(each[6]) , str(location)[2:-3])
            if each[6] == str(location)[2:-3]:
                new.append(each)
                #print(each)
