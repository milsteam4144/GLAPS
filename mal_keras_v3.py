...
#Get the data for only year 2011
df = df[:800]
...
def build_model():
    ...
    model.add(layers.Dense(400, activation='relu', input_shape=(train_data.shape[1],)))
...
epochs=400, batch_size=1, verbose=1)
test_mse_score, test_mae_score = model.evaluate(test_data, test_targets)

test_targets_predicted = model.predict(test_data)
print('predicted value: ', test_targets_predicted[1])
test_targets = np.array(test_targets)
print('actual value: ', test_targets[1])
